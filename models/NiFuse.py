...
# 假设的 model_urls 字典
model_urls = {
    'fuse_pretrained': '/home/bygpu/med/newmodel_weights.pth'  # 替换为实际的预训练权重 URL
}
...
def HiFuse_Base(num_classes: int,img_size=None):
    # def __init__(self, num_classes,img_size=None):
...
# 定义 SuperDAM 函数
def hifuse(num_classes=3, img_size=None, **kwargs):
    model = HiFuse_Base(num_classes=num_classes, img_size=img_size)
    return model

# 注册 SuperDAM 到 timm 模型注册表，并支持预训练权重加载
@register_model
def fuse(pretrained=False, **kwargs):
    model = hifuse(**kwargs)
    if pretrained:
        url = model_urls.get('fuse_pretrained')
        if url:
            load_pretrained_weights(model, url)
        else:
            print("No pretrained weights URL found for fuse.")
    return model

# 使用 create_model 函数创建 SuperDAM 模型
if __name__ == "__main__":
    model = create_model('fuse', pretrained=False, num_classes=3)
